...
export const TaskBoard = memo(function TaskBoard() {
  ...
  const onDragEnd = useCallback((event: DragEndEvent) => {
    ...
    const targetStatus: TaskStatus | undefined = COLUMN_ORDER.includes(overId as TaskStatus)
    ...
    updateDoc(doc(db, 'tasks', taskId), { status: targetStatus }).catch((err: unknown) => {
  ...
  return (
    <DndContext sensors={sensors} collisionDetection={closestCorners} onDragEnd={onDragEnd}>
      <div className="flex gap-4 overflow-x-auto pb-4" role="region" aria-label="Task board — drag cards between columns to change status">
