...
export const TaskColumn = memo(function TaskColumn({ status, tasks }: TaskColumnProps) {
  ...
  return (
    ...
    >
      <div className={cn('flex items-center gap-2 rounded-lg border px-3 py-2', HEADER_ACCENT[status], 'bg-navy-800/50')}>
        <span className={cn('h-2 w-2 rounded-full', HEADER_DOT[status])} aria-hidden="true" />
        <h2 className="text-sm font-semibold">{COLUMN_LABELS[status]}</h2>
        <span className="ml-auto rounded-full bg-navy-700 px-2 py-0.5 text-xs font-medium text-slate-400" aria-label={`${tasks.length} tasks`}>
          {tasks.length}
        </span>
      </div>
      ...
      >
        ...
        {tasks.length === 0 && (
          <p className="flex flex-1 items-center justify-center text-xs text-slate-600 italic py-8" aria-live="polite">Drop tasks here</p>
        )}
      </div>
    </section>
  );
